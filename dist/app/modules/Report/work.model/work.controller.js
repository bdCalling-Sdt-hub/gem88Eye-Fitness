...
exports.getAllDataByInstructorId = exports.getMilesDetails = exports.createMilesDetails = exports.getWorkDetails = exports.createWeek2WorkDetails = exports.createWeek1WorkDetails = exports.createWorkDetails = exports.getInstructorById = exports.getInstructors = exports.createInstructor = void 0;
const instructor_model_1 = __importDefault(require("./instructor.model"));
const workdetails_model_1 = __importDefault(require("./workdetails.model"));
const miles_model_1 = __importDefault(require("./miles.model"));
const mongoose_1 = __importDefault(require("mongoose"));
const isDateWithinPeriod = (instructorId, workDate) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const instructor = yield mongoose_1.default.model('Instructor').findById(instructorId);
        if (!instructor) {
...
const createInstructor = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const newInstructor = new instructor_model_1.default({
            periodBeginning,
            periodEnding,
            instructorName,
        });
        yield newInstructor.save();
        const populatedInstructor = yield newInstructor.populate('instructorName');
...
const getInstructorById = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { id } = req.params;
        const instructor = yield instructor_model_1.default.findById(id).populate('instructorName');
        if (!instructor) {
...
const isTwoWeeksLimitReached = (instructorId) => __awaiter(void 0, void 0, void 0, function* () {
    const twoWeeksAgo = new Date();
    twoWeeksAgo.setDate(twoWeeksAgo.getDate() - 14);
    const workCount = yield workdetails_model_1.default.countDocuments({
        instructor: instructorId,
        'workDetails.date': { $gte: twoWeeksAgo },
    });
    const milesCount = yield miles_model_1.default.countDocuments({
        'milesDetails.date': { $gte: twoWeeksAgo },
    });
    return workCount + milesCount >= 14;
});
const createWorkDetails = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { instructorId, workDetails } = req.body;
        const limitReached = yield isTwoWeeksLimitReached(instructorId);
        if (limitReached) {
            return res.status(400).json({ message: "Two-week limit reached. Cannot add more details." });
        }
        const isValidDate = yield isDateWithinPeriod(instructorId, workDetails.date);
        if (!isValidDate) {
            return res.status(400).json({
                message: "The work date must be within the instructor's period beginning and ending dates."
            });
        }
        const existingWorkDetail = yield workdetails_model_1.default.findOne({
            instructor: instructorId,
        ...
        const newWorkDetails = new workdetails_model_1.default({
            workDetails,
            instructor: instructorId,
        });
        yield newWorkDetails.save();
...
exports.createWorkDetails = createWorkDetails;
const createWeek1WorkDetails = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { instructorId, workDetails } = req.body;
        // Ensure that the instructor has a period beginning and ending date set
        const instructor = yield instructor_model_1.default.findById(instructorId);
        if (!instructor) {
...
exports.createWeek1WorkDetails = createWeek1WorkDetails;
const createWeek2WorkDetails = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { instructorId, workDetails } = req.body;
        const instructor = yield instructor_model_1.default.findById(instructorId);
        if (!instructor) {
...
exports.createWeek2WorkDetails = createWeek2WorkDetails;
const getWorkDetails = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { workDetailsId } = req.params;
        const workDetails = yield workdetails_model_1.default.find({ workDetailsId: workDetailsId })
            .populate('instructor')
...
const createMilesDetails = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const existingMilesCount = yield miles_model_1.default.countDocuments({ instructor: instructorId });
        let weekNumber;
        ...
        const newMilesDetails = new miles_model_1.default({
            milesDetails,
            instructor: instructorId,
            weekNumber: weekNumber,
        });
        yield newMilesDetails.save();
...
exports.createMilesDetails = createMilesDetails;
const getMilesDetails = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { milesDetailsId } = req.params;
        const milesDetails = yield miles_model_1.default.find({ milesDetailsId: milesDetailsId })
            .populate('instructor')
            .populate('instructor.instructorName')
            .exec();
...
exports.getMilesDetails = getMilesDetails;
const getAllDataByInstructorId = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        yield instructor.populate('instructorName');
        const workDetails = yield workdetails_model_1.default.find({ instructor: instructorId });
        const milesDetails = yield miles_model_1.default.find({ instructor: instructorId });
        const processedWorkDetails = workDetails.map((workDetail) => {
            ...
            return Object.assign(Object.assign({}, workDetail.toObject()), { totalAmount, date: new Date(workDetail.workDetails.date) });
        });
        const processedMilesDetails = milesDetails.map((mileDetail) => {
            ...
            return Object.assign(Object.assign({}, mileDetail.toObject()), { totalAmount, date: new Date(mileDetail.milesDetails.date) });
        });
        const periodBeginning = new Date(instructor.periodBeginning);
        const periodEnding = new Date(instructor.periodEnding);
        const weeklyData = [];
        ...
        const week1WorkDetails = processedWorkDetails.filter(detail => detail.date >= week1Start && detail.date <= week1End);
        const week2WorkDetails = processedWorkDetails.filter(detail => detail.date >= week2Start && detail.date <= week2End);
        // Filter miles details by weekNumber instead of date
        const week1MilesDetails = processedMilesDetails.filter(detail => detail.weekNumber === 1);
        const week2MilesDetails = processedMilesDetails.filter(detail => detail.weekNumber === 2);
        // Rest of your calculations remain the same
        const week1WorkingHours = week1WorkDetails.reduce((total, detail) => total + detail.workDetails.hours, 0);
        const week1Miles = week1MilesDetails.reduce((total, detail) => total + detail.milesDetails.miles, 0);
        ...
        const week1AvgMileageRate = week1Miles > 0
            ...
            : 0;
        const week1WorkAmount = week1WorkDetails.reduce((total, detail) => total + detail.totalAmount, 0);
        const week1MilesAmount = week1MilesDetails.reduce((total, detail) => total + detail.totalAmount, 0);
        // Week 2 calculations
        const week2WorkingHours = week2WorkDetails.reduce((total, detail) => total + detail.workDetails.hours, 0);
        const week2Miles = week2MilesDetails.reduce((total, detail) => total + detail.milesDetails.miles, 0);
        const week2WorkAmount = week2WorkDetails.reduce((total, detail) => total + detail.totalAmount, 0);
        const week2MilesAmount = week2MilesDetails.reduce((total, detail) => total + detail.totalAmount, 0);
        ...
        const totalBiweeklyWorkingHours = biweeklyWorkDetails.reduce((total, detail) => total + detail.workDetails.hours, 0);
        const totalBiweeklyWorkAmount = biweeklyWorkDetails.reduce((total, detail) => total + detail.totalAmount, 0);
        // Calculate average hour rate for biweekly period
        const biweeklyAvgHourRate = totalBiweeklyWorkingHours > 0
            ? biweeklyWorkDetails.reduce((total, detail) => total + detail.workDetails.hourRate * detail.workDetails.hours, 0) / totalBiweeklyWorkingHours
            : 0;
        const biweeklyData = {
...
exports.getAllDataByInstructorId = getAllDataByInstructorId;
